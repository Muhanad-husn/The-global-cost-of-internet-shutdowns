...
- **Dashboard shipped** at `app/streamlit_dashboard.py`, 4 tabs (World Map / Country Drill-down / Time Series / Top 10), persistent methodology-caveat banner via `st.warning` above the tab strip. All Plotly figures come from the S5 helpers (`world_choropleth`, `time_series`, `top10_bar`) and rollups (`country_cost_rollup`, `shutdown_day_rollup`, `monthly_event_series`) — zero duplicated viz code.
- **Sidebar controls:** year-range slider (2019–2025), shutdown-day view radio (combined/separated per Decision Log #14), country multi-select (empty = all). Country-grouping toggle (Decision #13) deferred — the WB income group / UN LDC / custom-regional lists live inline in `02_main.ipynb` §8 and weren't promoted to `src/` in S4. Adding them is a ~30-line lift; left for S7 robustness work or a future polish pass.
- **Country drill-down** picks the highest-cost country in the current filter window as its default, so the tab is informative on first click. Gantt timeline uses `px.timeline` with `end_for_plot` = `end_date` → `start_date + duration_days` → `start_date + 1d` fallbacks for unparseable/ongoing ends; bars colored by `type`. Cost metric in the country header dedupes on `(iso3, year)` before summing (Decision #19) — the Tab 4 caption explains why.
- **Top 10 tab** mirrors the hero figure (cost ranking) but exposes a shutdown-days toggle; under `separated` view it expands the bucket-level table beneath the bars so the platform-block dominance (Iran 96.7%, Russia 99.5%) is visible without leaving the dashboard.
- **Python environment gotcha.** Streamlit + plotly + the project package live in the **`portfolio` conda env** (`C:\Users\mou97\.conda\envs\portfolio\python.exe`). The Bash tool starts in **base** conda — `python -m streamlit run …` from base fails with `ModuleNotFoundError: No module named 'plotly'`. S7 reproducibility check must explicitly activate `portfolio` or invoke the env's python by full path.
- **Streamlit version in `portfolio` env: 1.57.0** (vs. 1.54.0 in base / user-level). No deprecation warnings observed. `st.warning` renders the persistent caveat banner well at desktop width; folds gracefully into a wider block on narrow viewports.
- **Screenshot pipeline** uses Playwright (chromium-headless-shell v148). `notebooks/_scratch/capture_dashboard_screenshot.py` waits for `h1` + `.js-plotly-plot` + a function gate (>50 country paths in the geo SVG) before snapping. Without the path-count gate, the capture fires while only the colorbar has rendered. Playwright is **not** in `pyproject.toml` deps — installed user-level for this one-off; if S7's reproducibility check wants the screenshot to regenerate automatically, add `playwright` to a `dev` extra and document `playwright install chromium`.
- **Screenshot at `figures/dashboard_screenshot.png`** — 1600×1100 viewport, ~200 KB, shows sidebar + caveat banner + World Map tab with `Total shutdown-days` metric, choropleth rendered (Russia + Iran + Pakistan + Myanmar visibly red).
- **Tests not updated** for the dashboard — Streamlit apps don't lend themselves to smoke testing without a running server. Coverage gap surfaces in S7 (alternative: a single test that imports the module and asserts `load_analytic()` returns >0 rows, no Streamlit runtime needed).
- **For S7:** the dashboard + screenshot are stable enough to reference from README's *How to reproduce* section, which has already been done. S7 still needs to fill in *Findings* and *Limitations* prose, populate the rationale-prose decision-table cells (already done by S4 — likely just needs cross-link tightening), and run the clean re-install reproducibility check.
